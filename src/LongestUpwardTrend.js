...
function LongestUpwardTrend(props){
    ...
    return(
        <div className="AnalysisContainer">
            <p>Amount of days closing price was higher than closing price the day before.</p>
            <p>Longest upward trend was {longestUpwardTrend.length} days.</p>
            {longestUpwardTrend.dates.map( elem => 
                <p key={elem.start.getTime()}>From {elem.start.toDateString()} to {elem.end.toDateString()}</p>)}
        </div>
    )
